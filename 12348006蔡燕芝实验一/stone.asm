...
loop1:
	dec word[count]				; 递减计数变量   判零转移
	jnz loop1					; >0：跳转;ZX每次运算后都会变化，你的程序中的JNZ代表上一次运算（DEC XXX）的结果

	mov word[count],delay
	dec word[dcount]				; 递减计数变量
    jnz loop1
	mov word[count],delay
	mov word[dcount],ddelay      ;function: delay

    mov al,1
    cmp al,byte[rdul]           ; equal ZF = 0
	jz  DnRt              ; jz ZF = 0才跳转
    mov al,2
    cmp al,byte[rdul]    
	jz  UpRt
