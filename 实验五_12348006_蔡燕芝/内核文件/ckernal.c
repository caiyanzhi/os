...
int n = 0;    /*获取的字符串的个数*/
char message[150] = "Welcom to MyOS v2.0 copyright (c) YZ Cai Time: 2014\\3\\24 \r\nType \"help\" for more infomations about commands\r\n";
...
void cmain(){
	...
	while(1){
		...
		{
			myprintf("here are four three programs in MyOS,numbered with a, b, c, d\r\n");
			myprintf("a is the user1.com which will test my int33 server in the kernal.\r\n");
			myprintf("b is the showOuch.com whith will show OUCH! in the sceen if you type the keyboard.\r\n");
			myprintf("c is the stome.com which will show the stome moving in the screen\r\n");
			...
			while(1)
			{
				...
				else if(mystrcmp(tmp, "run ",n,4) == 1)
				{
					int isQuit = 0;
					for(i = 4; i < n; i++)
						if((tmp[i] < 'a' || tmp[i] > 'c') && tmp[i] != ' ') /*判断要执行的程序是否符合要求*/
						{
							myprintf("your input couldn't be recognized,please try again.");
							isQuit = 1;
							break;
                    ...
                    for(i = 4; i < n; i++)
					{
						switch(tmp[i])
						{
							case 'a':
								...
								runTestMyInt33();
								cls();
								myprintf("showOuch.com has been excuted successfully!");
								toNextLine();
								break;
							case 'b':
								cls();
								runShowOuch();
								cls();
								myprintf("ShowMyInt.com has been excuted successfully!");
								toNextLine();
								break;
							case 'c':
								cls();
								runStone();
								cls();
								myprintf("stone.com has been excuted successfully!");
								toNextLine();
								break;
							case ' ':
								break;
