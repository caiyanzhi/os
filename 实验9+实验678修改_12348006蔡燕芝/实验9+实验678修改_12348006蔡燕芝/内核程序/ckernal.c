...
extern void loadsubProgram(int numOfsector, int zhutouhao,int zhumian, int qishishanqu);
extern int getline(char *mess);
extern void cls();
extern int runProg(char *filename);/*创建进程参数为程序的扇区数和其实扇区编码*/
...
int n = 0;    /*获取的字符串的个数*/
char message[150] = "Welcom to MyOS v2.2 copyright (c) YZ Cai Time: 2014\\6\\1 \r\nType \"help\" for more infomations about commands\r\n";
...
void getFileName(char *str1, char *name)
{
	i = 4;
	while(i < 12 && str1[i])
	{
...
void cmain(){
	...
	while(1){
		...
		else if(mystrcmp(tmp,"run",n,3) == 1)/*进入run分支*/
		{
			...
			myprintf("enter the program filename to run\r\n");
			myprintf("enter exit() to quit\r\n");
			toNextLine();
			while(1)
			{
				userInput();
