...
typedef enum PCB_Status{READY=0,EXIT,RUNNING, BLOCKED,SUSPENDED}PCB_Status;
...
void SaveCurrentPCB(int ax,int cx, int dx, int bx, int sp, int bp, int si, int di, int ds ,int es,int fs,int gs, int ss,int call_save,int ip, int cs,int fl)
{
	...
	pcb_list[CurrentPCBno].regImg.SP = sp + 24; /*恢复栈顶*/
...
int runProg(int numOfSectors, int startOfSectors)
{
	...
	LoadsubProgram(numOfSectors,startOfSectors);   /*加载进程到内存*/
	return pid;
}
...
{
	myprintf("Prog ");
	int2Chars(tmpMessage,CurrentPCBno);
	myprintf(tmpMessage);
	myprintf(" wait\r\n");
